...
static int bits_set(char *buf, int bytes) {
  int i, count;
  char work;

  for(i=count=0; i < bytes; i++) {
    work = buf[i];
    while(work) {
      work &= (work-1);
      count++;
...
/**
 * Consume Bitmap/Chunk data from an FD that we don't want
 *
 * @param f a file descriptor
 * @param pairs the number of bitmap/chunklist pairs to consume
 * @param chunk_size the size of a diff chunk for write-diff cmd
 **/
static void consume_diff(int f, int pairs, int chunk_size) {
  char buf[4096];
  int bitmap_size;

  bitmap_size = (4096 / chunk_size) >> 3;
  while (pairs-- > 0) {
    readit(f, buf, bitmap_size);
    readit(f, buf, chunk_size * bits_set(buf, bitmap_size));
  }
}
...
int mainloop(CLIENT *client) {
	...
	reply.error = 0;
	while (go_on) {
		char buf[BUFSIZE], bitmap[NBD_DIFF_MAX_BITMAP_SIZE];
    int j, chunk_size, bitmap_size;
    char *chunk;
		char* p;
		size_t len;
		size_t currlen;
...
next_request:
    ...
    case NBD_CMD_WRITEDIFF:
      ...
      if(bitmap_size == 0)
        ...
        int remaining;
			  DEBUG("net->bitmap, ");
				readit(client->net, bitmap, bitmap_size);
				DEBUG("net->buf, ");
        remaining = chunk_size * bits_set(bitmap, bitmap_size);
        readit(client->net, buf, remaining);
				...
				if ((client->server->flags & F_READONLY) ||
					...
					DEBUG("[WRITE to READONLY!]");
					ERROR(client, reply, EPERM);
          consume_diff(client->net, (len/4096)-1, chunk_size);
          goto next_request;
				}
        chunk = buf;
        for(j = 0; j < 4096; j += chunk_size) {
          if(bitmap[(j/chunk_size)/8] & (1 >> ((j/chunk_size)%8))) {
            if (expwrite(request.from + j, chunk, chunk_size, client,
                  request.type & NBD_CMD_FLAG_FUA)) {
              DEBUG("Write failed: %m" );
              ERROR(client, reply, errno);
              consume_diff(client->net, (len/4096)-1, chunk_size);
